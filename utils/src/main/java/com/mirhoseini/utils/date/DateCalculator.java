package com.mirhoseini.utils.date;

import java.util.Calendar;

import android.content.Context;

import com.mirhoseini.utils.R;
						...
						if (diffMinutes == 0) {
							result = diffSeconds
									...
									+ context.getResources().getString(
											R.string.utils__ago_sec);

						} else {
							result = diffMinutes
									+ " "
									+ context.getResources().getString(
											R.string.utils__ago_min);
						}
					} else {
						result = diffHours
								+ " "
								+ context.getResources().getString(
										R.string.utils__ago_hour);
					}
				} else {
					result = diffDays
							+ " "
							+ context.getResources()
									.getString(R.string.utils__ago_day);
				}
			} else {
				result = diffMonth + " "
						+ context.getResources().getString(R.string.utils__ago_month);
			}
		} else {
			result = diffYear + " "
					+ context.getResources().getString(R.string.utils__ago_year);
		}
